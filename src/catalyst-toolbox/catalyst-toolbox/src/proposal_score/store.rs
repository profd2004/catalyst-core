...
pub fn store_score_into_proposal(
    ...
    values.insert("alignment_score".to_string(), ((alignment_score.0 * 100.0).round() / 100.0).into());
    values.insert("feasibility_score".to_string(), ((feasibility_score.0 * 100.0).round() / 100.0).into());
    values.insert(
        "auditability_score".to_string(),
        ((auditability_score.0 * 100.0).round() / 100.0).into(),
    );
